...
function addGamesToPage(games) {
    ...
    for (const game of games) {
        ...
        <h3>Pledged: $${game.pledged.toLocaleString("en-US")}</h3>
        `
...
const desc = `$${totalRaised.toLocaleString("en-US")} has been raised for ${totalGames} game${totalGames > 1 ? 's' : ''}. Currently, ${unfunded.length}
game${unfunded.length > 1 ? 's are' : ' is'} unfunded.`;
...
const [first, second, ..._] = sortedGames
...
runnerUpPledge.innerText = second.name;
secondGameContainer.append(runnerUpPledge);